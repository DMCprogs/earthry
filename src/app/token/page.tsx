...
import { ButtonCustomSC, DivWrapperFormSC } from './styles.page';
...
const Form = () => {
    ...
    useEffect(() => {
        ...
        settokenContract(new _web3.eth.Contract(
            tokenABI,
            '0x31Db46A0B13436514ccd2b1A921FAC0c8B96942f'
        ))

        setemitterContract(new _web3.eth.Contract(
            emitterABI,
    ...
    const getGasPrice = async () => {
        try {
            const response = await axios.get('https://gpoly.blockscan.com/gasapi.ashx?apikey=key&method=pendingpooltxgweidata');
            console.log(response.data.result.rapidgaspricegwei.toString())
            return parseUnits(parseFloat(String((response.data.result.rapidgaspricegwei * 1.5))).toString(), 'gwei');
        } catch (error) {
    ...
    const tokenForETHQuote = async () => {
        let _sell: any = 0
        try {
            const {data} = await axios.get(`https://polygon.api.0x.org/swap/v1/quote?buyToken=MATIC&sellToken=0x6B22Cd88CA6Fa3E8429CfFcDFE914936a7EBeF15&sellAmount=100000000000`, {
                headers: {
    ...
    const ETHForTokenQuote = async () => {
        let _buy: any = 0
        try {
            const {data} = await axios.get(`https://polygon.api.0x.org/swap/v1/quote?buyToken=0x6B22Cd88CA6Fa3E8429CfFcDFE914936a7EBeF15&sellToken=MATIC&sellAmount=100000000000`, {
                headers: {
                    '0x-api-key': '25ee6597-bb0c-4048-b3c8-4d0bf1b61c53'
                }
            });
    ...
    const getConnection = async () => {
        return new Promise(async resolve => {
            ...
            const unsub = onSnapshot(doc(db, "codes", connectionDoc.id), async (_doc: any) => {
    ...
    const buyTokens = async (amount:any) => new Promise(async resolve => {
    ...
    const sellTokens = async (amount: any) => new Promise(async resolve => {
        ...
        try {
            ...
            await tokenContract.methods.approve('0xf43Fa9c52192998e3cda213005Af50bB6cA7875B', amountToken);
    ...
    return (
        <DivWrapperFormSC>
            <ButtonCustomSC
                onClick={onConnect}
            >Connect Wallet</ButtonCustomSC>
            <ButtonCustomSC
                onClick={onLogin}
            >Login</ButtonCustomSC>
            <ButtonCustomSC
                onClick={onBuy}
            >Buy Tokens</ButtonCustomSC>
            <ButtonCustomSC
                onClick={onSell}
            >Sell Tokens</ButtonCustomSC>
            <ButtonCustomSC
                onClick={Logout}
            >Logout</ButtonCustomSC>
        </DivWrapperFormSC>
    );
};
