...
const ModalProject: FC<ModalProps> = ({Profits}) => {
    ...
    return (
        ...
        <DivModalContainerSC> <DivContainerGrafic>
            ...
            <DivHeightfixSC>
                <DivNumberBoxSC><DivCourceTitleImgSC path={courceImg.src}></DivCourceTitleImgSC>
                    <div>{amount}</div>
                </DivNumberBoxSC>
                <DivSliderSC>
                    <Slider
            ...
            <DivContainerGrafic2>
                <DivContainerInput>
                    ...
                    <DivTexStacingSC style={{fontSize: '18px'}}>Select the percentage of APY to maintain the project
                        (min 1%)</DivTexStacingSC>
                    <div style={{display: 'flex', alignItems: 'center', gap: '10px'}}><InputPercent
                        onChange={Percen}><DivContainer><DivNumbers>{trackDatas}</DivNumbers>
                        <div style={{display: 'flex', flexDirection: 'column', gap: '5px',}}><DivPlus
                            onClick={Plus}><BsArrowUpShort size={20}/></DivPlus>
                            <DiMunus onClick={Minus}><BsArrowDownShort size={20}/></DiMunus></div>
                    </DivContainer></InputPercent><BsPercent size={28}/></div>
                </DivContainerInput>
                <DivContainerInput>
                    <DivTexStacingSC style={{fontSize: '18px'}}>You donate to the project in the
                        project</DivTexStacingSC>

                    <div style={{display: 'flex', alignItems: 'center', gap: '10px'}}><InputPercent2
                        onChange={Percen}><DivContainer2><DivNumbers2>{amount}</DivNumbers2>
                        <div style={{display: 'flex', flexDirection: 'column', gap: '5px',}}><DivPlus
                            onClick={PlusTokens}><BsArrowUpShort size={20}/></DivPlus>
                            <DiMunus onClick={MinusTokens}><BsArrowDownShort size={20}/></DiMunus></div>
                    </DivContainer2></InputPercent2><DivText>Earthy tokens</DivText></div>
